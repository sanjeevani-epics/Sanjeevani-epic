...
contract EHR { 
  ...
  function addRecord(string memory _cid, string memory _fileName, address _patientId, string memory _metadata) public senderIsDoctor patientExists(_patientId) {
    ...
    require(c.isAuthorized == true && c.validUntil > block.timestamp, "Doctor does not have active consent to upload records for this patient");
  ...
  function getConsentedDoctors(address _patientId) public view patientExists(_patientId) returns (Consent[] memory) {
      ...
      for(uint i = 0; i < doctorAddresses.length; i++) {
          if (patientConsents[_patientId][doctorAddresses[i]].isAuthorized && 
